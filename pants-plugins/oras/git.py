...
async def get_git_info(
    request: GitInfoRequest, root: pants.base.build_root.BuildRoot
) -> GitInfo:
    git_paths = await pants.engine.rules.Get(
        pants.core.util_rules.system_binaries.BinaryPaths,
        pants.core.util_rules.system_binaries.BinaryPathRequest(
            binary_name="git", search_path=["/usr/bin", "/bin"]
        ),
    )
    ...
    return GitInfo(commit_hash=git_commit_hash, git_tags=tuple(git_tags))
